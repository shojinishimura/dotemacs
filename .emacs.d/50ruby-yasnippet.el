...
;   ("return" "return ${value}" "return value" nil)
   ...
   ("require" "require \"$0\"" "require \"file\"" nil)
...
;   ("rescue.q" "rescue ${exp}" "rescue qualifier" (not (ruby-starting "rescue")))
   ...
   ("part" "partition do |${item}|\n  $0\nend" "partition do |item| ... end" nil)
   ("part1" "partition {|${item}|$0}" "partition {|item| ...}" nil)
   ("array" "Array.new(${10}) do |${i}|\n  $0\nend" "Array.new(10) do |i| ... end" nil)
   ("array1" "Array.new(${10}) {|${i}|$0}" "Array.new(10) {|i| ...}" nil)
   ("hash" "Hash.new do |${hash}, ${key}|\n${hash}[${key}] = $0\nend" "Hash.new do |hash, key| hash[key] = ... end" nil)
   ("hash1" "Hash.new {|${hash}, ${key}| ${hash}[${key}]=$0}" "Hash.new {|hash, key| hash[key]=...}" nil)
